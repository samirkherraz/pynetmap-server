...
class ZabbixAPI(object):
    ...
    def _setuplogging(self):
        self.logger = logging.getLogger(
            "zabbix_api.%s" % self.__class__.__name__)
    ...
    def login(self, user='', password='', save=True):
        ...
        hashed_pw_string = "md5(" + \
            hashlib.md5(l_password.encode('utf-8')).hexdigest() + ")"
        self.debug(logging.DEBUG, "Trying to login with %s:%s" %
                   (repr(l_user), repr(hashed_pw_string)))
        obj = self.json_obj(
            'user.login', {'user': l_user, 'password': l_password}, auth=False)
    ...
    def test_login(self):
        if self.auth != '':
            obj = self.json_obj('user.checkAuthentication',
                                {'sessionid': self.auth}, auth=False)
            result = self.do_request(obj)
    ...
    def do_request(self, json_obj):
        ...
        if self.httpuser:
            ...
            auth = 'Basic ' + \
                string.strip(base64.encodestring(
                    self.httpuser + ':' + self.httppasswd))
        ...
        request = urllib2.Request(
            url=self.url, data=json_obj.encode('utf-8'), headers=headers)
        if self.proto == "https":
            if HAS_SSLCONTEXT and not self.validate_certs:
                https_handler = urllib2.HTTPSHandler(
                    debuglevel=0, context=_create_unverified_context())
            else:
    ...
    def api_version(self, **options):
        obj = self.do_request(self.json_obj(
            'apiinfo.version', options, auth=False))
        return obj['result']
